...
class ResNetExtractor(Conv2dExtractor):
    def __init__(
            self,
            input_dim: int,
            in_channels: int = 1,
        ...
        super(ResNetExtractor, self).__init__(input_dim=input_dim, activation=activation)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.conv = MaskCNN(
            nn.Sequential(
                ResidualBlock(in_channels, out_channels[0]),
                ResidualBlock(out_channels[0], out_channels[0]),
                nn.MaxPool2d(2, stride=2),
                ResidualBlock(out_channels[0], out_channels[1]),
                ResidualBlock(out_channels[1], out_channels[1]),
                nn.MaxPool2d(2, stride=2),
            )
        )
...
class DeepSpeech2(EncoderModel):
    ...
    def __init__(
        ...
        self.device = device
        self.conv = ResNetExtractor(input_dim, activation=activation)
        self.rnn_layers = nn.ModuleList()
        rnn_output_size = rnn_hidden_dim << 1 if bidirectional else rnn_hidden_dim
        print("aaaa")
        print(num_rnn_layers)
